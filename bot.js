...
client.on("ready", () => {
  ...
  client.user.setActivity(`-yardım | Toplam ${client.guilds.cache.size} Sunucu`);
})
...
client.login(process.env.token) //token adında bir .env oluşturun ve içine botunuzun tokenini yazın 

  client.on("ready", () => {
  client.channels.cache.get("896870410254184518").join();//sesli bi kanala katılacagı icin kanal ID si
}) 
...
.addField("Sunucudaki Kişi Sayısı:", guild.memberCount)
   ...
   client.channels.get('821330892718342163').send(rrrsembed);
